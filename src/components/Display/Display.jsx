...
function Display({ onGroupingSelect, onOrderingSelect }) {
    ...
    const handleClickOutside = (event) => {
        if (event.target.closest('.Display') === null) {
            setIsOpen(false);
        }
    };
    useEffect(() => {
    ...
    return (
        <div className={`Display ${theme}`}>
            <button className={`dsp-btn ${theme}`} onClick={toggleBox}><img className='display-icon' src={images.displayicon} alt="" />Display <img className='drop-icon' src={images.dropicon} alt="" /></button>
            {isOpen && (
